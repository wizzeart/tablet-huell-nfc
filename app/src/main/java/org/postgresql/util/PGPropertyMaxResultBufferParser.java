...
/**
 * Reemplazo compatible con Android de la clase homónima de pgjdbc 42.2.27.
 *
 * <p>La versión original usa {@code java.lang.management.ManagementFactory}
 * (API de Java SE / JMX) para calcular el máximo de heap. Esa clase NO existe
 * en el runtime de Android (ART), por lo que la conexión JDBC fallaba con
 * {@code NoClassDefFoundError: Ljava/lang/management/ManagementFactory;}
 * durante {@code PGStream.setMaxResultBuffer()} en cada conexión.
 *
 * <p>Esta copia es idéntica byte a byte en su lógica salvo que reemplaza
 * {@code ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getMax()}
 * por {@code Runtime.getRuntime().maxMemory()}, que devuelve el mismo valor
 * (máximo de heap) y SÍ está disponible en Android.
 *
 * <p>La clase original fue eliminada del jar {@code postgresql-42.2.27-android.jar}
 * (en app/libs) para evitar el error "type defined multiple times" en D8/R8.
 */
public class PGPropertyMaxResultBufferParser {
  ...
  /** Máximo de heap disponible para la VM. Equivalente Android-safe de
  ...
  private static long calculatePercentOfMemory(String value, int percentPhraseLength) {
    ...
    long result = (long) (percent * maxHeapMemory());
    return result;
  }
  ...
  private static long adjustResultSize(long value) {
    if (value > 0.9 * maxHeapMemory()) {
      long newResult = (long) (0.9 * maxHeapMemory());
